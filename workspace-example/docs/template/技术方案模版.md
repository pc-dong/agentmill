...
| 后端原型（可选） | `demo-backend-service` `feature/<卡号>-…` | 见 **§7**：领域段（模型 + UseCase 接口）/ 接口段（Controller + SpringDoc）；**非**业务实现 |
...
> 经人确认后可跑 **backend-prototype 领域段**：在 `demo-backend-service` 对应 domain lib **仅**落领域数据结构 + `port.in` UseCase **接口**签名；**禁止** UseCase Impl / Repository / MyBatis / Liquibase。  
...
> 总览表；字段级契约以 **SpringController / 生成 HTML** 为准（见 §7）。变更类型与备注供评审与差分阅读。  
...
| 段 | 时机 | 产出 | 禁止 |
|----|------|------|------|
| **A 领域原型** | TECH Phase 4 可选；或用户明确「后端领域原型」 | domain 实体/VO/枚举等**数据结构**；`port.in.*UseCase` **接口**（方法 + Command/Query） | UseCase Impl、Repository、MyBatis、Liquibase、Controller |
| **B 接口原型** | TECH Phase 6 可选；或用户明确「后端接口原型 / 出 API 文档」 | `Spring*Controller` 路由 + SpringDoc；port Controller / Request·Result 接口；DTO + `@Schema`；方法体 **stub** | UseCase Impl、Mapper/Entity 落库、完整 Audit/MQ 业务链路 |
| **正式实现** | 另开 writing-plans / executing-plans | Impl、持久化、Liquibase 生产脚本、api-test | — |

触发约定：

- 用户只说「改原型 / 更新领域模型与 API」且未要求实现 → **默认只做 A+B 薄契约**，完成后停住，再问是否开实现计划。  
- 增量字段（例 P-xxx 加属性）：原型仍只改模型字段 + 接口/DTO 签名；**不要**顺手改 `*UseCaseImpl` / `*RepositoryImpl` / Entity / Liquibase。  
- 前后端同卡：**同名** feature 分支；后端仓 `demo-backend-service`：`feature/<卡号>-<slug>`（自 `origin/develop` 拉出）。
...
| 归属 app / domain lib | 例：`demo-content-app` / `demo-content-lib` |
| 领域模型 / UseCase 接口 paths | 段 A：`…/model/…`、`…/port/in/…UseCase.java` |
| SpringController paths | 段 B：`…/SpringXxxController.java` 列表 |
| Port / DTO paths | 可选：presentation port + app DTO |
| API 文档 HTML | `docs/api-doc/demo-<app>-…-YYYYMMDD.html`（未导出则空） |
| 状态 | 未做 / Draft（有 stub） / 已导出（有 HTML） |
...
| ✅ 允许 | ❌ 禁止（除非用户明确要求完整实现） |
|---------|--------------------------------------|
| 领域数据结构与 `port.in` UseCase 接口签名 | UseCase **Impl**（含改已有 Impl 接新字段） |
| SpringController + SpringDoc；DTO `@Schema` | Repository / MyBatis Entity·Mapper·XML |
| Controller 方法体 `UnsupportedOperationException("prototype")` 或 `return null`（统一一种） | Liquibase / 生产库 ALTER（DDL **草稿**仍写在 `tech/ddl/`，不在本原型 skill 落生产脚本） |
...
- [ ] §5 API 列表齐全（含 **事件监听 MQ 表**）；若有原型则 HTTP 与 SpringController / HTML path 一致  
